        ...
        for transit in trials_data[episode]:
            ...
            if Q_LEARNING:
                ...
                if trial_end_state == new_state:
                    ...
                    target = gamma * np.max(q_value[new_state])
                delta = target - q_value[now_state, action_chosen]
                q_value[now_state, action_chosen] += alpha * delta
            elif step != 1:
                target = 0 + gamma * q_value[now_state, action_chosen]
                delta = target - q_value[last_state, last_action]
                q_value[last_state, last_action] += alpha * delta
...
if __name__ == "__main__":
    ...
    MF_fit_result = open("MF_no_reinit_fit_result.csv", "w")
    ...
    for lists in os.listdir("data/"):
        ...
        condition = [(MF_no_reinit_lld, initial, bounds) for initial in initial_value]
