...
def simulate_MF_no_reinit(randomized=True,
                          alpha=0.1,
                          tau=1.0,
                          repeat=50,
                          gamma=1,
    ...
    if path:
        ...
        dir_path = "data/MF_no_reinit/alpha%.1f_tau%.1f/" % (alpha, tau) + ("randomized" if randomized else "block") + "/"
    ...
    for time in range(0, repeat):
        ...
        for trial in trials:
            ...
            while now_state != trial_end_state:
                ...
                delta = target - q_value[now_state, action]
                q_value[now_state, action] += alpha * delta
